...
SELECT "id", "userName" FROM p where "deletedAt" IS NULL;`;
  ...
  childUsers.map(async (child) => {
    let childUser = await this.getUserById(child.id, ["id", "userBlock", "betBlock", "roleName"]);
    if (userBlock == 1 || betBlock == 1) {
      ...
      if (betBlock == 1 && userBlock == 0) {
        childUser.userBlock = userBlock;
        let blockByOtherUser = await userBlockRepo.findOne({ where: { userId: child.id, createBy: Not(createBy), userBlock: true } });
        if (blockByOtherUser) {
      ...
      if (userBlock == 0 && betBlock == 0) {
        ...
        let blockUser = await userBlockRepo.find({ where: { userId: child.id } });
    ...
    user.update({ id: childUser.id }, { betBlock: childUser.betBlock, userBlock: childUser.userBlock });
