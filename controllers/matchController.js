  ...
  try {
    ...
    if (redisData) {
      ...
      if (matchesBetsByUsers?.length) {
        try {
          const redisMatch = await commonGetMatchDetailsFromRedis(Array.from(matchIds).join(","));
          if (redisMatch?.data) {
            matchDetails = redisMatch;
          }
          else {
